...
object TMDB {
  case class ImdbLookup(imdbID: String) extends XmlTask[List[Movie]] {
...
trait TmdbIntegrator { // extends Task[Movie] with Logging { // self: Search[Movie] with Logging =>
  
  //val logID = "TmdbIntegrator(" + self.id + ")"
  
//  abstract override def execute(): List[Movie] = {
